...
invoke() { # $1=agent $2=model $3=provider $4=prompt
  ...
  case "$agent" in
    ...
    hermes)
      if [[ -n "$provider" ]]; then
        out="$(timeout "$ATTEMPT_TIMEOUT" hermes --provider "$provider" -m "$model" \
          --in "$WORKDIR" --yolo -z "$prompt" </dev/null 2>&1)" || rc=$?
      else
        out="$(timeout "$ATTEMPT_TIMEOUT" hermes -m "$model" --in "$WORKDIR" --yolo \
          -z "$prompt" </dev/null 2>&1)" || rc=$?
      fi ;;
    *) return 3 ;;
  esac
  printf '%s' "$out"
...
for row in "${CHAIN[@]}"; do
  ...
  attempt=$((attempt+1))
  t0=$(date +%s%3N); rc=0
  out="$(invoke "$agent" "$model" "$provider" "$PROMPT")" || rc=$?
