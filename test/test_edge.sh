...
bash "${LEGACY_DIR}/handoff.sh" task-001 capture >/dev/null 2>&1
...
bash "${LEGACY_DIR}/compress.sh" task-001 >/dev/null 2>&1   # first compress (marks compressed)
out=$(bash "${LEGACY_DIR}/compress.sh" task-001 2>&1); rc=$?
...
: > "${ORCH_DIR}/runner.log"
bash "${LEGACY_DIR}/runner.sh" >/dev/null 2>&1   # run_all_tasks: nothing should be available
