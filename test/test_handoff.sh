...
# capture idempotent
out=$(bash "${REPO_DIR}/handoff.sh" task-001 capture 2>&1); assert_eq "capture exits 0" "$?" "0"
...
out=$(bash "${REPO_DIR}/handoff.sh" task-001 get 2>&1); assert_contains "get reports no deps" "$out" "No dependencies"
...
bash "${REPO_DIR}/handoff.sh" task-001 compress --force >/dev/null 2>&1
...
out=$(bash "${REPO_DIR}/handoff.sh" task-X full 2>&1)
